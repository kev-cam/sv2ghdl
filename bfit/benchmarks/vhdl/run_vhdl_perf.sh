...
echo "and the fork now trades blows with a release four versions newer: it"
echo "leads or ties half the rows (2026-08-29 regenerate: b14 and b17"
echo "outright, b12 via the parallel scheduler's first qualifying cell, b22"
echo "via l3d — the 4-state engine winning rows outright while carrying MORE"
echo "per-wire semantics), trails b01/b06 ~10% and the tiny bench_seq."
echo "Which side of parity a given row lands on moves a few percent run to"
echo "run; the month-scale trend is what the mechanism list below records."
echo "Fused dispatch is default-on and the native projection complete: \`bench_comb\`"
