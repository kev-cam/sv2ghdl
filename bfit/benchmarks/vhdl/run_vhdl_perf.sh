...
echo "and the fork now LEADS stock on SIX of eight rows (b12 +13%, b17 +11%,"
echo "bench_comb +6%, b22 +5%, b01/b06 ahead) with fused dispatch default-on and"
echo "the native-projection pilot landed: \`bench_comb\`"
...
echo "native projection (eebbeae60: the fast-driver waveform schedule inlined"
echo "into generated code — the runtime spec specialized against elaborated"
echo "structure; instructions −6% to −14%) took six rows past stock.  The"
echo "holdouts: bench_seq 1.08x, and b14 1.10x — integer-heavy, its odd-size"
echo "ports decline the inline path (the FASTn follow-up)."
echo
